...
func (gq *GroupQuery) QueryPermissions() *PermissionQuery {
	query := (&PermissionClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) WithPermissions(opts ...func(*PermissionQuery)) *GroupQuery {
	query := (&PermissionClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withPermissions; query != nil {
...
func (gq *GroupQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Group, init func(*Group), assign func(*Group, *Permission)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Group)
	nids := make(map[uuid.UUID]map[*Group]struct{})
	for i, node := range nodes {
