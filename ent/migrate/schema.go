...
var (
	...
	DeploymentsColumns = []*schema.Column{
		...
		{Name: "status", Type: field.TypeEnum, Enums: []string{"build-pending", "build-queued", "build-running", "build-succeeded", "build-cancelled", "build-failed", "active", "waiting", "crashing", "removed"}},
	...
	ServiceConfigsColumns = []*schema.Column{
		...
		{Name: "railpack_provider", Type: field.TypeEnum, Nullable: true, Enums: []string{"node", "deno", "bun", "go", "java", "php", "python", "ruby", "rust", "elixir", "staticfile", "unknown"}},
		...
		ForeignKeys: []*schema.ForeignKey{
			{
				...
				Columns:    []*schema.Column{ServiceConfigsColumns[31]},
