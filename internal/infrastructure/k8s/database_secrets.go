...
func (self *KubeClient) SyncDatabaseSecretForService(ctx context.Context, service *ent.Service) error {
	...
	case "postgres":
		url = fmt.Sprintf("postgresql://%s:%s@%s:%d/postgres?sslmode=disable", username, password, service.Name, 5432)
	case "redis":
		url = fmt.Sprintf("redis://%s:%s@%s-headless:%d", "default", password, service.Name, 6379)
	}
